...
class TileContainer : VerticalLayout {
        ...
        switch(evt.action) {
            case MouseAction.ButtonDown:
                if (evt.button == MouseButton.Left) {
                    src.setFlags(WidgetFlags.Capturing);  // Use widget flags for capture
                    return true;
                }
                break;
            case MouseAction.ButtonUp:
                if (evt.button == MouseButton.Left) {
                    src.resetFlags(WidgetFlags.Capturing);  // Reset capture flag
                    return true;
                }
                break;
            case MouseAction.Move:
                if (evt.button == MouseButton.Left) {
                    ...
                    auto newPos = Point(
                        winRect.left + evt.x - evt.x,
                        winRect.top + evt.y - evt.y
                    );
    ...
    // Add missing snapToZone function
    private Rect snapToZone(Rect winRect, SnapZone zone) {
        return zone.bounds;  // Simple implementation - just snap to zone bounds
    }
}
class NotesVaultWindow : Window {
    ...
    this() {
        ...
        try {
            ...
            // Set default font
            FontManager.instance.setDefaultFont("Arial", 12);
...
class InteractiveTerminal : VerticalLayout {
    ...
    private bool onInputChange(EditableContent content) {
        string input = content.text.to!string;
    ...
    private void executeCommand(string cmd) {
        ...
        switch (parts[0]) {
            ...
            case "exit":
                if (auto win = cast(Window)this)  // Cast this instead of parent
                    win.close();
                break;
    ...
    private void appendOutput(string text) {
        outputArea.text = outputArea.text ~ text.to!dstring ~ "\n"d;
        // Use vertical scroll position
        if (auto scrollable = cast(ScrollWidget)outputArea.parent) {
            scrollable.vscrollPos = scrollable.maxVScrollPos;
